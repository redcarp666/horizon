...
public class IterableUtils {
	 ...
	 */
	@SafeVarargs
	public static <E, C extends Collection<E>> C collectionOf(final IntFunction<C> supplier, final E... objects) {
	 ...
	 */
	public static <E, C extends Collection<E>> C collectionOf(final Supplier<C> supplier, final Iterator<? extends E> it) {
		if (it == null) {
			return supplier.get();
		} else {
	...
	public static <E> Iterable<E> concat(final Iterable<? extends E>... inputs) {
		AssertionUtils.shouldNotNull(inputs);
		return new Iterable<E>() {
			...
			public Iterator<E> iterator() {
				return Stream.of(iterables).map(it -> StreamSupport.stream((Spliterator<E>) it.spliterator(),
				                                                           false)).reduce(Stream::concat).orElseGet(
	...
	public static <E> Iterator<E> concat(final Iterator<? extends E>... inputs) {
		...
		return new Iterator<E>() {
			@SuppressWarnings("unchecked")
			final Iterator<E>[] iterators = StreamUtils.streamOf(inputs).filter(ObjectUtils::isNotNull).toArray(Iterator[]::new);
			int index = 0;
	...
	public static <E> E get(final Iterable<? extends E> iterable, final int index) {
		...
		if (iterable instanceof List) {
			List<E> list = ObjectUtils.forceCast(iterable);
	...
	public static <E> Iterable<E> iterableOf(final Enumeration<? extends E> enums) {
		return enums == null ? emptyIterable() : () -> new Iterator<E>() {
	...
	public static <T> int search(T[] a, T key) {
		if (a != null) {
			int len = a.length;
			for (int i = 0; i < len; i++) {
				if (ObjectUtils.areEqual(a[i], key)) {
					return i;
				}
			}
		}
	 ...
	 */
	public static <S, T> Iterable<T> transform(final Iterable<S> it, final Function<? super S, ? extends T> converter) {
	 ...
	 * @param converter the conversion function
	 */
	public static <S, T> Iterator<T> transform(final Iterator<S> it, final Function<? super S, ? extends T> converter) {
		return it == null ? emptyIterator() : new Iterator<T>() {
			final Iterator<S> fromIterator = it;
			final Function<? super S, ? extends T> useConvert = ObjectUtils.defaultObject(converter,
			                                                                              ObjectUtils::forceCast);
